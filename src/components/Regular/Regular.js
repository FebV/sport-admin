...
export default class Regular extends React.Component {
    ...
    query() {
        ...
        this.isLoading = true;
        RegularModel.getPublishedRegular(this.page, 20)
            .then(res => {
                this.isLoading = false;
    ...
    render() {
        return (
                ...
                {this.state.comments.map( (ele, idx) => {
                    if(ele.picture)
                        return (
                            ...
                            <Link to={`/Regular/${ele.id}`}>
                            <img width={900} height={500} alt="900x500" src={`${API.base}Regular/picture/path/${ele.picture}`} />
                            <Carousel.Caption>
                                <h3>{ele.title}</h3>
                            </Carousel.Caption>
                            </Link>
            ...
            <List>
                {this.state.comments.map( (ele, idx) => {
                    return (
                        <Link to={`/Regular/${ele.id}`} key={idx}>
