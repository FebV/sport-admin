...
import RegularModel from '../../controllers/Regular';

export default class RegularDetail extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            article: '新闻加载中',
    ...
    componentDidMount() {
        RegularInfoModel.getRegularDetail(this.props.match.params.id)
            .then(res => {
                this.setState(res);
            })
    }
    ...
    render() {
        return (
            <div style={{width: "100%", display: "flex", justifyContent: "center", marginTop: "2vh"}}>
                ...
                <div style={{padding: "30px"}} className="regularContent" dangerouslySetInnerHTML={{__html: this.state.article}}></div>
                </div>
            </div>
        )
    }
